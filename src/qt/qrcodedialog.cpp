...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("INK")
    updateDisplayUnit();
}
...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non INK unit input in lnReqAmount, we generate the URI with INK as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::INK, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
